...
| `--global` | Also scan user skills (`$CLAUDE_CONFIG_DIR/skills` or `~/.claude/skills`, `~/.codex/skills`, `~/.commandcode/skills`, `~/.agents/skills`, `$GROK_HOME/skills`, `~/.codeium/windsurf/skills`), Claude user settings / agents / `CLAUDE.md` / rules / commands and `~/.claude.json` (user + matching local `mcpServers`), Copilot `$COPILOT_HOME` / `~/.copilot/hooks` and `settings.json`, Codex `$CODEX_HOME` / `~/.codex/config.toml`, Grok `$GROK_HOME` / `~/.grok` (hooks + `config.toml`), Command Code user MCP/agents/memory, and Windsurf `~/.codeium/windsurf` MCP + `global_rules.md` + `hooks.json` (see below) |
...
Adds user skill dirs (`$CLAUDE_CONFIG_DIR/skills` or `~/.claude/skills`, `~/.codex/skills`,
`~/.commandcode/skills`, `~/.agents/skills`, `$GROK_HOME/skills` or
`~/.grok/skills`), Claude user settings / agents / `CLAUDE.md` / rules /
commands and `~/.claude.json` (top-level user `mcpServers` plus local
`projects.<scanned-path>.mcpServers`), Copilot `~/.copilot/hooks`, Codex `$CODEX_HOME` /
...
`memories/global_rules.md`, `hooks.json`, and `skills` to the structural checks: a `SKILL.md` that
is malformed is malformed wherever it lives, and those findings carry a
`source: global` evidence entry so you can tell them apart. Codex and
...
`auth.json` / `mcp_credentials.json`. A `~/.claude.json` without user or
matching local `mcpServers` is skipped, not reported as unreadable.
`CLAUDE_CONFIG_DIR` relocates the `~/.claude` directory; `~/.claude.json`
...
reads the project settings files, in-tree plugin and command-file hooks,
...
| `~/.claude/settings.json` (or `$CLAUDE_CONFIG_DIR/settings.json`) under `--global` | scanned project root, `${CLAUDE_PROJECT_DIR}` |
...
| `SKILL.md` frontmatter (including in-tree plugin skills) | the skill's own directory, then the project root |
| `.claude/commands/*.md` frontmatter | the command file's directory, then the project root |
...
- **Two Claude hook locations stay unread.** Managed policy settings and
  ...
  missing script, is not reported. User settings, skills, agents,
  `CLAUDE.md`, rules, and commands under `$CLAUDE_CONFIG_DIR` or
  `~/.claude` are scanned only under `--global`. User `~/.claude.json`
  MCP is scanned only under `--global`: top-level `mcpServers` and
  `projects.<scanned-path>.mcpServers`. Other projects' local servers
  are skipped. A file without those keys is skipped, not
  `config.unreadable`. In-tree plugin skills, agents, commands, and
  `.mcp.json` are read; plugin agent frontmatter hooks are ignored
...
- **Windsurf Devin CLI MCP and auto memories are unread.** Cascade MCP is
  ...
  `global_rules.md` are not opened. System / enterprise rule, hook, and
  skill directories (`/etc/devin/rules`, `/etc/windsurf/rules`,
  `/etc/windsurf/hooks.json`, `/etc/windsurf/skills/`) stay unread. There
  is no quoted project MCP path. JetBrains `~/.codeium/hooks.json` is unread.
